...
:cli-up (
    echo ^> LDMud service startup

    @rem check docker container exist or not
    docker ps -a > .dockerps.log
    for /f "tokens=*" %%p in ('findstr /R ldmud-service .dockerps.log') do (
    ...
    if NOT defined IS_CONTAINER_EXIST (
        echo ^> Build LDMud service image
        docker build -t ldmud .\docker\ldmud

        echo ^> Run LDMud service
        docker run -d --rm ^
            -p 5000:5000 ^
            --name ldmud-service ^
            ldmud

        echo ^> Use Telnet to connecnt ldmud-service with port 5000
    ) else (
        echo ^> ldmud-service is working.
    )
    goto end
)
...
:cli-down (
    echo ^> LDMud service close down
    docker rm -f ldmud-service
    goto end
)
